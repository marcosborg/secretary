    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.groups.update", [$group->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number') ? 'is-invalid' : '' }}" type="text" name="number" id="number" value="{{ old('number', $group->number) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('overseer') ? 'is-invalid' : '' }}" name="overseer_id" id="overseer_id">
                    @foreach($overseers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('overseer_id') ? old('overseer_id') : $group->overseer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('helper') ? 'is-invalid' : '' }}" name="helper_id" id="helper_id">
                    @foreach($helpers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('helper_id') ? old('helper_id') : $group->helper->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
