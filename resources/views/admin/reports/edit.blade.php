        ...
        <form method="POST" action="{{ route("admin.reports.update", [$report->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('month') ? 'is-invalid' : '' }}" name="month_id" id="month_id" required>
                    @foreach($months as $id => $entry)
                        <option value="{{ $id }}" {{ (old('month_id') ? old('month_id') : $report->month->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('month'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('publisher') ? 'is-invalid' : '' }}" name="publisher_id" id="publisher_id" required>
                    @foreach($publishers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('publisher_id') ? old('publisher_id') : $report->publisher->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('preached') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="preached" id="preached" value="1" {{ $report->preached || old('preached', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('publications') ? 'is-invalid' : '' }}" type="number" name="publications" id="publications" value="{{ old('publications', $report->publications) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('videos') ? 'is-invalid' : '' }}" type="number" name="videos" id="videos" value="{{ old('videos', $report->videos) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('hours') ? 'is-invalid' : '' }}" type="number" name="hours" id="hours" value="{{ old('hours', $report->hours) }}" step="1" required>
                @if($errors->has('hours'))
                    <div class="invalid-feedback">
                        {{ $errors->first('hours') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('revisits') ? 'is-invalid' : '' }}" type="number" name="revisits" id="revisits" value="{{ old('revisits', $report->revisits) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('studies') ? 'is-invalid' : '' }}" type="number" name="studies" id="studies" value="{{ old('studies', $report->studies) }}" step="1" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('pioneer') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="pioneer" id="pioneer" value="1" {{ $report->pioneer || old('pioneer', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('observations') ? 'is-invalid' : '' }}" name="observations" id="observations">{{ old('observations', $report->observations) }}</textarea>
