    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.meetings.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Meeting::WEEK_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('week') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="week_{{ $key }}" name="week" value="{{ $key }}" {{ old('week', '1') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('presences') ? 'is-invalid' : '' }}" type="number" name="presences" id="presences" value="{{ old('presences', '0') }}" step="1" required>
