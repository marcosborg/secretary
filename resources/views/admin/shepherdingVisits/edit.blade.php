...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.shepherdingVisit.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.shepherding-visits.update", [$shepherdingVisit->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('group') ? 'is-invalid' : '' }}" name="group_id" id="group_id" required>
                    @foreach($groups as $id => $entry)
                        <option value="{{ $id }}" {{ (old('group_id') ? old('group_id') : $shepherdingVisit->group->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('group'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('publisher') ? 'is-invalid' : '' }}" name="publisher_id" id="publisher_id" required>
                    @foreach($publishers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('publisher_id') ? old('publisher_id') : $shepherdingVisit->publisher->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('datetime') ? 'is-invalid' : '' }}" type="text" name="datetime" id="datetime" value="{{ old('datetime', $shepherdingVisit->datetime) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('shepherding_reason') ? 'is-invalid' : '' }}" name="shepherding_reason_id" id="shepherding_reason_id" required>
                    @foreach($shepherding_reasons as $id => $entry)
                        <option value="{{ $id }}" {{ (old('shepherding_reason_id') ? old('shepherding_reason_id') : $shepherdingVisit->shepherding_reason->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('observations') ? 'is-invalid' : '' }}" name="observations" id="observations">{!! old('observations', $shepherdingVisit->observations) !!}</textarea>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('accomplished') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="accomplished" id="accomplished" value="1" {{ $shepherdingVisit->accomplished || old('accomplished', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('next_visit') ? 'is-invalid' : '' }}" type="text" name="next_visit" id="next_visit" value="{{ old('next_visit', $shepherdingVisit->next_visit) }}">
