...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.lifeMinistryEvent.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.life-ministry-events.update", [$lifeMinistryEvent->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('life_ministry') ? 'is-invalid' : '' }}" name="life_ministry_id" id="life_ministry_id" required>
                    @foreach($life_ministries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('life_ministry_id') ? old('life_ministry_id') : $lifeMinistryEvent->life_ministry->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('assignment') ? 'is-invalid' : '' }}" name="assignment_id" id="assignment_id" required>
                    @foreach($assignments as $id => $entry)
                        <option value="{{ $id }}" {{ (old('assignment_id') ? old('assignment_id') : $lifeMinistryEvent->assignment->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('student') ? 'is-invalid' : '' }}" name="student_id" id="student_id" required>
                    @foreach($students as $id => $entry)
                        <option value="{{ $id }}" {{ (old('student_id') ? old('student_id') : $lifeMinistryEvent->student->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
