...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.responsibility.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.responsibilities.update", [$responsibility->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.responsibility.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $responsibility->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('pioneer') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="pioneer" id="pioneer" value="1" {{ $responsibility->pioneer || old('pioneer', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('tik_pioneer') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="tik_pioneer" id="tik_pioneer" value="1" {{ $responsibility->tik_pioneer || old('tik_pioneer', 0) === 1 ? 'checked' : '' }}>
