    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.receipts.update", [$receipt->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $receipt->date) }}" required>
                @if($errors->has('date'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('worldwide_work') ? 'is-invalid' : '' }}" type="number" name="worldwide_work" id="worldwide_work" value="{{ old('worldwide_work', $receipt->worldwide_work) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('local_congregation_expenses') ? 'is-invalid' : '' }}" type="number" name="local_congregation_expenses" id="local_congregation_expenses" value="{{ old('local_congregation_expenses', $receipt->local_congregation_expenses) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('other') ? 'is-invalid' : '' }}" type="number" name="other" id="other" value="{{ old('other', $receipt->other) }}" step="0.01">
                @if($errors->has('other'))
                    <div class="invalid-feedback">
                        {{ $errors->first('other') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('completed_by') ? 'is-invalid' : '' }}" name="completed_by_id" id="completed_by_id" required>
                    @foreach($completed_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('completed_by_id') ? old('completed_by_id') : $receipt->completed_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('verified_by') ? 'is-invalid' : '' }}" name="verified_by_id" id="verified_by_id" required>
                    @foreach($verified_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('verified_by_id') ? old('verified_by_id') : $receipt->verified_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
