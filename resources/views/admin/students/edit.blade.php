            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Student::RESPONSIBILITY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('responsibility') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="responsibility_{{ $key }}" name="responsibility" value="{{ $key }}" {{ old('responsibility', $student->responsibility) === (string) $key ? 'checked' : '' }} required>
